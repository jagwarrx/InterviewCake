...
def color_graph(graph, colors):
    for node in graph:
        if node in node.neighbors:
            raise Exception("Only legal coloring!")
        if node.color == None:
            neighbor_colors = get_neighbors_colors(node)
        
        if(neighbor_colors == None):
            node.color = colors[1]
        else:
            for color in colors:
...
def get_neighbors_colors(root):
    neighbor_colors = []
    for neighbor in root.neighbors:
        neighbor_colors.append(neighbor.color)
